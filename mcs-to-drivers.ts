...
for (const csvName of allCsvFiles) {
	...
	console.log(`\n\n========== ${csvName} has ${sanatizedMCs.length * batchSize} MCs. Extracting data ==========\n\n`);
	for (let i = 0; i < sanatizedMCs.length; i++) {
		await Promise.all(sanatizedMCs[i].map(async (MC) => {
			await getDriverData(MC);
		}));
		if (i % 10 === 0 && i !== 0) {
			fs.writeFileSync(`./drivers_data/${csvName.split(".")[0]}.json`, JSON.stringify(driversData, null, 2));
		}
	}
	fs.writeFileSync(`./drivers_data/${csvName.split(".")[0]}.json`, JSON.stringify(driversData, null, 2));
...
async function getDriverData(MCNumber: string) {
	...
	while (USDOTRequestResponse === "Request was blocked") {
		console.error(`\x1b[31mRequest was blocked for MC-${MCNumber}. Retrying in ${retryTime} minutes \x1b[0m`);
	...
	const parsedUSDOTRequestResponse = new JSDOM(USDOTRequestResponse).window.document;
	const usdotSelector = parsedUSDOTRequestResponse.querySelector<HTMLElement>('body > p > table > tbody > tr:nth-child(2) > td > table > tbody > tr:nth-child(2) > td > center:nth-child(3) > table > tbody > tr:nth-child(5) > td:nth-child(2)');
	if (!usdotSelector) {
		console.error(`\x1b[31mNo data for MC-${MCNumber} \x1b[0m`);
		return;
	}
	...
	if (!driverDataRequest) {
		console.error(`\x1b[31mNo registration box for MC-${MCNumber} USDOT-${usdot} \x1b[0m`);
		return;
	}
	...
	result['Link'] = `https://ai.fmcsa.dot.gov/SMS/Carrier/${usdot}/CarrierRegistration.aspx`;

	// **2. Extract Carrier Information (Left Column)**
	const col1Items = node.querySelectorAll('ul.col1 li');
	col1Items.forEach(li => {
		const label = li.querySelector('label')?.textContent?.replace(':', '').trim();
		const value = li.querySelector<HTMLElement>('span.dat')?.textContent?.trim();
		if (label == "Address") {
			result[label || ''] = value?.replace(/\t/g, '').replace(/\n/g, '').replace(/\s{2,}/g, ' ');
		} else {
			result[label || ''] = value;
		}
	});
...
async function fetchUSDOT(currentMC: string) {
	try {
		...
		const request = await fetch(`https://safer.fmcsa.dot.gov/query.asp`, requestData)
		if (!request.ok) {
		...
		console.error(`\x1b[31mError fetching USDOT for MC-${currentMC}. Your internet connection may be down. \x1b[0m`);
		return null;
	}
}
...
async function fetchDriverData(usdot: number) {
	try {
		const request = await fetch(`https://ai.fmcsa.dot.gov/SMS/Carrier/${usdot}/CarrierRegistration.aspx`)
		const response = await request.text();
		return response;

	} catch (error) {
		console.error(`\x1b[31mError fetching driver data for USDOT-${usdot}. Your internet connection may be down. \x1b[0m`);
		return null;
	}
}